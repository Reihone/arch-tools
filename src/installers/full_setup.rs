...
pub async fn run_full_setup(architecture: arch::Architecture) -> Result<(), String> {
    let theme = ColorfulTheme::default();

    println!(
        "{} Architecture: {}",
        "[i]".cyan(),
        format!("{}", architecture).cyan().bold()
    );
    println!();
    ...
    gpu::show_gpu_menu(architecture).await?;

    println!("{}", "\n[2/5] Setting up Desktop Environment...".bold());
    desktop::show_desktop_menu(architecture).await?;

    println!("{}", "\n[3/5] Installing applications...".bold());
    apps::show_apps_menu(architecture).await?;

    println!("{}", "\n[4/5] Installing browsers...".bold());
    browsers::show_browsers_menu(architecture).await?;

    println!("{}", "\n[5/5] Setting up package managers...".bold());
    package_managers::show_pm_menu(architecture).await?;
